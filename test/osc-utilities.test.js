...
(function () {
  ...
  test("fromOscMessage with no type string works", function () {
    ...
    assert.strictEqual(
      translate != null ? translate.address : void 0,
      "/stuff"
    );
  ...
  test("fromOscBundle works with no messages", function () {
    ...
    return assert.deepEqual(
      translate != null ? translate.elements : void 0,
      []
    );
  });
  ...
  test("applyMessageTranformerToBundle fails on bundle without tag", function () {
    ...
    return assert.throws(function () {
      return func(
        osc.concat([
          osc.toOscString("#grundle", osc.toIntegerBuffer(0, "Int64")),
        ])
      );
    });
  });
  ...
  test("timestamp <-> timeTag round trip", function () {
    ...
    return assert(
      near(osc.timetagToTimestamp(osc.timestampToTimetag(now)), now)
    );
  });
